...
PYTHON_BIN="${PYTHON_BIN:-$SKILLZERO_ENV/bin/python}"
PYTHON_TRAIN_BIN="${PYTHON_TRAIN_BIN:-$SKILLZERO_ENV/bin/python}"
MODEL_PATH="${MODEL_PATH:-$MODEL_ROOT/Qwen3.5-4B}"
DATA_ROOT="${DATA_ROOT:-$DATA_HOME/nq_hotpot_qwen35_fc_lora_variants}"
CKPT_ROOT="${CKPT_ROOT:-$CKPT_HOME/nq_hotpot_qwen35_fc_lora_sweep_2gpu}"
LOG_DIR="${LOG_DIR:-$LOG_HOME/nq_hotpot_qwen35_fc_lora_sweep_2gpu}"
NQ_SOURCE_DIR="${NQ_SOURCE_DIR:-$DATA_HOME/nq_qwen35_fc_general_top10000_run1}"
HOTPOT_SOURCE_DIR="${HOTPOT_SOURCE_DIR:-$DATA_HOME/hotpotqa_qwen35_fc_general_top10000_run1}"
...
prepare_dataset_variants() {
    local dataset="$1"
    local src_dir="$2"
    local out_root="$DATA_ROOT/$dataset"
    ...
    if [ -f "$with_skill_dir/train.parquet" ] && [ -f "$with_skill_dir/val_1000.parquet" ] && \
       [ -f "$no_skill_dir/train.parquet" ] && [ -f "$no_skill_dir/val_1000.parquet" ]; then
        log "Dataset variants already prepared for $dataset"
        return 0
    fi

    log "Preparing dataset variants for $dataset from $src_dir"
...
run_one() {
    ...
    local rank="$3"
    local variant="$4"
    local data_dir="$DATA_ROOT/$dataset/$variant"
    local alpha
    alpha="$(rank_to_alpha "$rank")"
    local tag="${dataset}_qwen35_fc_${variant}_lora_r${rank}_ep${TOTAL_EPOCHS}"
    ...
    while IFS= read -r attempt; do
...

        {
            ...
            echo "dataset=$dataset variant=$variant rank=$rank alpha=$alpha max_length=$MAX_LENGTH micro_bsz=$micro_bsz train_bsz=$train_bsz lr=$LR epochs=$TOTAL_EPOCHS"
        ...
        if CUDA_VISIBLE_DEVICES="$gpu" \
            ...
            "$PYTHON_TRAIN_BIN" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=1 \
                ...
                trainer.project_name=SkillZero-nq-hotpot-qwen35-fc-sft-lora-sweep \
...
worker_dataset() {
    local dataset="$1"
    log "Start dataset sweep: $dataset"
    for rank in $RANKS; do
        run_one 0 "$dataset" "$rank" with_skill &
        pid0=$!
        run_one 1 "$dataset" "$rank" no_skill &
        pid1=$!
        wait "$pid0"
        wait "$pid1"
    done
    log "Finished dataset sweep: $dataset"
}

prepare_dataset_variants "nq" "$NQ_SOURCE_DIR"
prepare_dataset_variants "hotpotqa" "$HOTPOT_SOURCE_DIR"

log "Start Qwen3.5-4B FC LoRA sweep for NQ and HotpotQA"
worker_dataset "nq"
worker_dataset "hotpotqa"
log "All Qwen3.5-4B FC LoRA sweep trainings finished"
