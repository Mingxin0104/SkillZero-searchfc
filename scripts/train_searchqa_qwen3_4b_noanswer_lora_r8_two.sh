...
LOG_DIR="${LOG_DIR:-/workspace/limingxin/logs/searchqa_qwen3_4b_noanswer_lora_r8_two}"
    ...
    for attempt in "${attempts[@]}"; do
        ...
        if CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" \
            ...
            "$TORCHRUN_BIN" --standalone --nnodes=1 --nproc_per_node="$NPROC_PER_NODE" \
                ...
                trainer.project_name=SkillZero-searchqa-qwen3-4b-noanswer-sft \
...
log "Start Qwen3-4B noanswer r8 two trainings"
run_once "qwen3_4b_noanswer_with_skill_lora_r8_ep${TOTAL_EPOCHS}" "$WITH_SKILL_DATA_DIR"
run_once "qwen3_4b_noanswer_no_skill_lora_r8_ep${TOTAL_EPOCHS}" "$NO_SKILL_DATA_DIR"
log "All Qwen3-4B noanswer r8 trainings finished"
