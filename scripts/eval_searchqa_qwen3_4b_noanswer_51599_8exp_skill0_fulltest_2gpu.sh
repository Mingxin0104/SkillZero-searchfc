...
LOG_DIR="${LOG_DIR:-/workspace/limingxin/logs/searchqa_qwen3_4b_noanswer_51599_8exp_skill0_fulltest_2gpu}"
...
RETRIEVER_PORT_A="${RETRIEVER_PORT_A:-8000}"
RETRIEVER_PORT_B="${RETRIEVER_PORT_B:-8001}"
RETRIEVER_LOG_A="${RETRIEVER_LOG_A:-$LOG_DIR/retrieval_server_gpu${GPU_A}.log}"
RETRIEVER_LOG_B="${RETRIEVER_LOG_B:-$LOG_DIR/retrieval_server_gpu${GPU_B}.log}"
...
RAY_TMP_ROOT="${RAY_TMP_ROOT:-/workspace/limingxin/rt/qf2}"

mkdir -p "$MERGED_ROOT" "$LOG_DIR" "$RAY_TMP_ROOT"

RUN_SPECS=(
  "qwen3_4b_base_fulltest|base|$BASE_MODEL"
  "qwen3_4b_noanswer_51599_with_skill_lora_r8_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_with_skill_lora_r8_ep1"
  "qwen3_4b_noanswer_51599_no_skill_lora_r8_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_no_skill_lora_r8_ep1"
  "qwen3_4b_noanswer_51599_with_skill_lora_r16_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_with_skill_lora_r16_ep1"
  "qwen3_4b_noanswer_51599_no_skill_lora_r16_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_no_skill_lora_r16_ep1"
  "qwen3_4b_noanswer_51599_with_skill_lora_r32_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_with_skill_lora_r32_ep1"
  "qwen3_4b_noanswer_51599_no_skill_lora_r32_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_no_skill_lora_r32_ep1"
  "qwen3_4b_noanswer_51599_with_skill_lora_r64_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_with_skill_lora_r64_ep1"
  "qwen3_4b_noanswer_51599_no_skill_lora_r64_ep1_fulltest|adapter|qwen3_4b_noanswer_51599_no_skill_lora_r64_ep1"
)
...
ensure_retriever() {
    local retriever_gpu="$1"
    local retriever_port="$2"
    local retriever_log="$3"
    local health_url="http://127.0.0.1:${retriever_port}/docs"
    resolve_retriever_model
    if curl -fsS -m 2 "$health_url" >/dev/null 2>&1; then
        log "Retriever already available on GPU $retriever_gpu port $retriever_port"
        return 0
    fi

    log "Starting retriever on GPU $retriever_gpu port $retriever_port faiss_gpu=$RETRIEVER_FAISS_GPU"
    ...
    log "Retriever is ready on GPU $retriever_gpu port $retriever_port"
}
...
ensure_shared_gpu_retriever() {
    ...
    log "Starting shared GPU retriever on GPUs ${RETRIEVER_VISIBLE_DEVICES} port ${RETRIEVER_SHARED_PORT} faiss_gpu=true"
    local -a retriever_args=(
        "$REPO_ROOT/examples/search/retriever/retrieval_server.py"
        --index_path "$RETRIEVER_INDEX_PATH"
        --corpus_path "$RETRIEVER_CORPUS_PATH"
    ...
    CUDA_VISIBLE_DEVICES="$RETRIEVER_VISIBLE_DEVICES" nohup "$RETRIEVER_PYTHON" "${retriever_args[@]}" > "$RETRIEVER_SHARED_LOG" 2>&1 &
    wait_for_retriever "$health_url" 360 5
...
eval_one() {
    ...
    local search_url="$4"
    local short_name
    short_name="$(printf '%s' "$run_name" | sed 's/qwen3_4b_//g; s/_with_skill/ws/g; s/_no_skill/ns/g; s/_noanswer_//g; s/_51599//g; s/_lora_r/r/g; s/_ep1//g; s/_fulltest//g; s/base/b/g')"
    local short_tmp
    short_tmp="$(printf '%s' "$short_name" | cut -c1-6)"
    local ray_tmp="${RAY_TMP_ROOT}/${gpu}_${short_tmp}"

    rm -rf "$LOG_DIR/${run_name}.generations" "$ray_tmp"
    mkdir -p "$ray_tmp"

    log "Start eval $run_name on GPU $gpu"
    RAY_DEDUP_LOGS=0 \
...
run_pair() {
    ...
    if [ "$kind_a" = "base" ]; then
        ...
        model_path_a="$MERGED_ROOT/$value_a"
    fi
    ...
    if [ "$idx_b" -ge 0 ]; then
        ...
        if [ "$kind_b" = "base" ]; then
            ...
            model_path_b="$MERGED_ROOT/$value_b"
        fi
...
log "SearchQA qwen3-4b noanswer 51599 full-test 2gpu eval started"
...
log "All SearchQA qwen3-4b noanswer 51599 full-test 2gpu evals finished"
