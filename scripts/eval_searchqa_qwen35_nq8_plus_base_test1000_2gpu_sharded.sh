...
QWEN35_CKPT_ROOT="${QWEN35_CKPT_ROOT:-$CKPT_HOME/nq_hotpot_qwen35_fc_lora_sweep_2gpu_run2}"
QWEN35_MERGED_ROOT="${QWEN35_MERGED_ROOT:-$MERGED_HOME/nq_qwen35_fc_lora_sweep_2gpu_run2_searchqa_eval}"
QWEN35_SERVE_ROOT="${QWEN35_SERVE_ROOT:-$SERVED_HOME/nq_qwen35_fc_lora_sweep_2gpu_run2_searchqa_eval}"

LOG_DIR="${LOG_DIR:-$LOG_HOME/searchqa_qwen35_nq8_plus_base_test1000_2gpu_sharded}"
DATA_ROOT="${DATA_ROOT:-$DATA_HOME/searchR1_processed_direct_skill0fmt}"
TEST_FILE="${TEST_FILE:-$DATA_HOME/nq_hotpot_qwen35_skill0fmt_input_10k1k/nq_test_1000.parquet}"
...
eval_one() {
    ...
    local session="sq35eval_gpu${gpu}"
    local vllm_log="$LOG_DIR/${run_name}.vllm.log"
    local eval_log="$LOG_DIR/${run_name}.eval.log"
    local base_url="http://127.0.0.1:${port}/v1"
    local short_name
    short_name="$(printf '%s' "$run_name" | sed 's/_test1000//g; s/nq_qwen35_fc_/q35_/g; s/_with_skill/_ws/g; s/_no_skill/_ns/g; s/_lora_r/r/g; s/_ep1//g; s/qwen35_4b_base/q35b/g')"
    local short_tmp
...
log "SearchQA Qwen3.5 base + NQ8 test1000 eval started"
...
log "All SearchQA Qwen3.5 base + NQ8 test1000 evals finished"
