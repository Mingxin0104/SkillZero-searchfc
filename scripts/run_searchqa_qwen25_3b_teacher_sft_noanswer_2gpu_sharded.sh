...
BASE_OUT_DIR="${BASE_OUT_DIR:-/workspace/limingxin/data/searchqa_qwen25_3b_teacher_sft_noanswer}"
MERGED_OUT_DIR="${MERGED_OUT_DIR:-$BASE_OUT_DIR}"
MERGED_NOSKILL_OUT_DIR="${MERGED_NOSKILL_OUT_DIR:-${BASE_OUT_DIR}_noskill}"
LOG_ROOT="${LOG_ROOT:-/workspace/limingxin/logs/searchqa_qwen25_3b_teacher_sft_noanswer_2gpu}"
SESSION_PREFIX="${SESSION_PREFIX:-searchqa_qwen25_noanswer_2gpu}"
...
start_shard() {
    ...
    tmux new-session -d -s "$session" \
        "cd '$REPO_ROOT' && MODEL_PATH='$MODEL_PATH' INPUT_PATH='$INPUT_PATH' CUDA_VISIBLE_DEVICES='$gpu' TENSOR_PARALLEL_SIZE=1 GPU_MEMORY_UTILIZATION='${GPU_MEMORY_UTILIZATION:-0.62}' START='$start' LIMIT='$limit' OUT_DIR='$out_dir' NOSKILL_OUT_DIR='$noskill_dir' BATCH_SIZE='${BATCH_SIZE:-128}' NUM_CANDIDATES='${NUM_CANDIDATES:-4}' MAX_STEPS='${MAX_STEPS:-5}' MAX_NEW_TOKENS='${MAX_NEW_TOKENS:-160}' RETRIEVAL_WORKERS='${RETRIEVAL_WORKERS:-64}' bash scripts/run_searchqa_qwen3_teacher_sft_noanswer.sh 2>&1 | tee '$log_file'"
}
