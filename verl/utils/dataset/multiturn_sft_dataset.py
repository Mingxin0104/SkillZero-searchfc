...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        def series_to_item(ls):
            ...
            if isinstance(ls, dict):
                return {k: series_to_item(v) for k, v in ls.items()}
            if isinstance(ls, list):
                return [series_to_item(v) for v in ls]
            if isinstance(ls, pandas.core.series.Series):
                return [series_to_item(v) for v in ls.tolist()]
    ...
    def _normalize_messages(cls, messages):
        ...
        for message in messages:
            ...
            if "reasoning" in message and "reasoning_content" not in message and isinstance(message["reasoning"], str):
                message["reasoning_content"] = message["reasoning"]

            tool_calls = message.get("tool_calls")
            if isinstance(tool_calls, dict):
                message["tool_calls"] = [cls._normalize_tool_call(tool_calls)]
            elif isinstance(tool_calls, list):
                message["tool_calls"] = [cls._normalize_tool_call(tool_call) for tool_call in tool_calls]
    ...
    @staticmethod
    def _to_1d_input_ids(tokens):
        if isinstance(tokens, torch.Tensor):
            return tokens[0] if tokens.dim() > 1 else tokens
        if hasattr(tokens, "input_ids"):
            input_ids = tokens.input_ids
            if isinstance(input_ids, torch.Tensor):
                return input_ids[0] if input_ids.dim() > 1 else input_ids
            return torch.tensor(input_ids[0] if isinstance(input_ids, list) and input_ids and isinstance(input_ids[0], list) else input_ids, dtype=torch.long)
        ...
        raise TypeError(f"Unsupported token container type: {type(tokens)}")

    def __getitem__(self, item):
        tokenizer = self.tokenizer
        messages = self._normalize_messages(self.messages[item])
        ...
        input_ids = self._to_1d_input_ids(full_tokens)
        ...
        try:
            ...
            for i, msg in enumerate(messages):
                ...
                start_pos = self._to_1d_input_ids(prev_tokens).shape[0] if prev_tokens is not None else 0
                end_pos = self._to_1d_input_ids(prefix_tokens).shape[0]

                # If this is an assistant message, set loss mask
                if msg["role"] == "assistant":
                    loss_mask[start_pos:end_pos] = 1
        except Exception:
            # Some tool-calling chat templates do not support arbitrary message prefixes.
            # Fall back to training on the full rendered conversation while preserving
            # the original function-calling message structure.
            loss_mask = attention_mask.clone().long()
