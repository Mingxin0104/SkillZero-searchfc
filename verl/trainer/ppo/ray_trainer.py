...
class RayPPOTrainer:
    ...
    def _dump_trajectory_metrics(self, data_sources, rewards, tool_callings, traj_uids, dump_path):
        """Dump one row per trajectory for reliable search-rate/accuracy summaries."""
        os.makedirs(dump_path, exist_ok=True)
        filename = os.path.join(dump_path, f"{self.global_steps}.trajectory_metrics.jsonl")
        ...
        with open(filename, "w") as f:
            for idx in unique_idx:
                entry = {
                    ...
                    "score": float(rewards[idx].item() if hasattr(rewards[idx], "item") else rewards[idx]),
                    "tool_callings": float(tool_callings[idx].item() if hasattr(tool_callings[idx], "item") else tool_callings[idx]),
                    "searched": bool(tool_callings[idx] > 0),
                    "is_success": bool(rewards[idx] > 0),
                }
                f.write(json.dumps(entry, ensure_ascii=False) + "\n")

        print(f"Dumped trajectory metrics to {filename}")
    ...
    def _validate_once(
        ...
        if validation_data_dir and not skip_generation_logging:
            dump_path = os.path.join(validation_data_dir, mp)
