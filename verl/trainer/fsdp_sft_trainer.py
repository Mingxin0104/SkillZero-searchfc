...
class FSDPSFTTrainer:
    ...
    def _prune_old_checkpoints(self):
        max_keep = self.config.trainer.get("max_ckpt_to_keep", None)
        ...
        if self.device_mesh.get_rank() == 0:
            ...
            for name in os.listdir(ckpt_root):
                ...
                if os.path.isdir(path) and re.match(r"global_step_\d+$", name):
                    ckpt_dirs.append(path)

            ckpt_dirs.sort(key=extract_step)
            for stale_path in ckpt_dirs[:-max_keep]:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(
                ...
                if global_step >= self.total_training_steps:
                    # Perform final validation unless explicitly disabled for train-only runs.
                    if not self.config.trainer.get("skip_validation", False):
                        val_losses = []
