...
def process_image(image, max_pixels: int = 2048 * 2048, min_pixels: int = 28 * 28, min_side: int = 28):
    ...
    if image.width < min_side or image.height < min_side:
        resize_factor = max(min_side / image.width, min_side / image.height)
        width = max(min_side, int(math.ceil(image.width * resize_factor)))
        height = max(min_side, int(math.ceil(image.height * resize_factor)))
        image = image.resize((width, height))
