...
class Encoder:
    ...
    def _encode_on_device(self, query_list: List[str], is_query: bool, device: str) -> np.ndarray:
    ...
    def encode(self, query_list: List[str], is_query=True) -> np.ndarray:
        try:
            return self._encode_on_device(query_list, is_query=is_query, device=self.device)
        except RuntimeError as exc:
            err = str(exc)
            if self.device != "cuda" or (
                "CUBLAS_STATUS_ALLOC_FAILED" not in err and "out of memory" not in err.lower()
            ):
                raise

            warnings.warn(
                "Dense retriever query encode hit GPU allocation failure; falling back to CPU for this request."
            )
            torch.cuda.empty_cache()
            self.model = self.model.to("cpu")
            self.device = "cpu"
            try:
                return self._encode_on_device(query_list, is_query=is_query, device="cpu")
            finally:
...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        ...
        if config.faiss_gpu:
            ...
            co.useFloat16 = True
            co.shard = True
            resources = [faiss.StandardGpuResources() for _ in range(faiss.get_num_gpus())]
            for res in resources:
                if os.environ.get("FAISS_GPU_NO_TEMP_MEMORY", "1") == "1":
                    res.noTempMemory()
                else:
                    temp_mem_gb = float(os.environ.get("FAISS_GPU_TEMP_MEM_GB", "4"))
                    res.setTempMemory(int(temp_mem_gb * 1024**3))
